...
def plot_menu(final_route: str) -> None:
    print(
        "\bThis script is going to move files from Download and Desktop folders into ",
        final_route,
    )
...
original_number_of_images = len(
    os.listdir(os.path.expanduser(images_destiny))
)  # save for statistics
    ...
    for file in current_files:
        try:
            ...
            if (
                ...
                os.rename(
                    src=directory + "/" + original_name, dst=directory + "/" + new_name
                )
                # Move file
                try:
                    shutil.move(
...
print(
    "Number of pictures after the process:  ",
    final_number_of_images,
    " (+",
    (final_number_of_images - original_number_of_images)
...
print(
    "{:<10} {:<10} {:<10} {:<10} {:<10}".format(
        cols_names[0], cols_names[1], cols_names[2], cols_names[3], cols_names[4]
    )
)
j = 23
for i in data_to_display_images:
    print(
        "{:<10} {:<10} {:<10} {:<10} {:<10}".format(
            i,
            data_to_display_images[i][0],
            data_to_display_images[i][1],
            data_to_display_images[i][2],
            data_to_display_images[i][3],
        )
    )
